...
yearly AS (
  SELECT
    ...
    100.0 * SUM(export_value * export_value_cagr) / NULLIF(SUM(export_value), 0)
      AS export_value_cagr_5y_pct,
    100.0 * SUM(export_weight * export_weight_cagr) / NULLIF(SUM(export_weight), 0)
      AS export_weight_cagr_5y_pct,
    100.0 * SUM(unique_traders * unique_traders_cagr) / NULLIF(SUM(unique_traders), 0)
      AS exporters_cagr_5y_pct
...
SELECT
  ...
  ROUND(export_value_cagr_5y_pct, 1) AS export_value_cagr_5y_pct,
  ROUND(export_weight_cagr_5y_pct, 1) AS export_weight_cagr_5y_pct,
  ROUND(exporters_cagr_5y_pct, 1) AS exporters_cagr_5y_pct
FROM yearly
